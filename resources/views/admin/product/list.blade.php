...
@section('content')
    <div id="content" class="container-fluid">
        <div class="card">
            ...
            <div class="card-body">
                <div class="analytic">
                    <a href="{{ request()->fullUrlWithQuery(['status' => '1']) }}" class="text-primary">Còn hàng<span
                            class="text-muted">({{ $count[0] }})</span></a>
                    <a href="{{ request()->fullUrlWithQuery(['status' => '0']) }}" class="text-primary">Hết hàng<span
                            ...
                            @if ($products->total() > 0)
                                ...
                                @foreach ($products as $product)
                                    ...
                                    <tr class="">
                                        ...
                                        <td>{{ $t }}</td>
                                        <td><img style="width: 80px; height: auto;" src="{{ asset($product->avatar) }}"
                                        ...
                                        @endif
                                        <td><a href="{{ route('edit_product', $product->id) }}"
                                                class="btn btn-success btn-sm rounded-0 text-white" type="button"
                                                data-toggle="tooltip" data-placement="top" title="<small>Sửa</small>"><i
                                                    class="fa fa-edit"></i></a>
                                            <a href="{{ route('add_thumbnail', $product->id) }}"
                                                class="btn btn-success btn-sm rounded-1 text-white" type="button"
                                                data-toggle="tooltip" data-placement="top"
                                                title="<small>Thêm ảnh sản phẩm</small>"><i class="fas fa-images"></i></a>
                                            ...
                                            @else
                                                <a href="{{ route('forceDelete_product', $product->id) }}"
