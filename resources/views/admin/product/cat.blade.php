                            ...
                            @if (!empty($result))
                                ...
                                @foreach ($result as $item)
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$item->slug}}</td>
                                        <td>{{ $count_post[$item->name] }}</td>
                                        <td>{{ $item->created_at }}</td>
                                        <td><a href="{{route('edit_product_cat', $item->id)}}" class="btn btn-success btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="<small>Sửa</small>"><i class="fa fa-edit"></i></a>
                                            <a href="{{route('delete_product_cat', $item->id)}}" onclick="return confirm('Bạn có chắc chắn xóa danh mục này? Mọi sản phẩm và danh mục con nằm trong danh mục này cũng sẽ bị xóa !')"
                                                class="btn btn-danger btn-sm rounded-0 text-white" type="button" data-toggle="tooltip" data-placement="top" title="<small>Xóa</small>"><i class="fa fa-trash"></i></a></td>
                                    </tr>
                                @endforeach
                            @else
